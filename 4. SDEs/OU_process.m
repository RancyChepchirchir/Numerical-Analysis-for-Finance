...
%sdev = sigma*sqrt(dt); % plain Euler
sdev = sigma*sqrt((1-exp(-2*alpha*dt))/(2*alpha)); % Euler with analytic moments

% Compute and accumulate the increments
for i = 1:nsteps
    %X(i+1,:) = X(i,:) + alpha*(mu-X(i,:))*dt + sdev*N(i,:); % plain Euler
    X(i+1,:) = mu+(X(i,:)-mu)*exp(-alpha*dt) + sdev*N(i,:); % Euler with a. m.
end
...
%% Plot the variance
figure(2)
plot(t,sigma^2/(2*alpha)*(1-exp(-2*alpha*t)),'r',t,sigma^2*t,'g',t,sigma^2/(2*alpha)*ones(size(t)),'b',t,var(X,0,2),'m')
